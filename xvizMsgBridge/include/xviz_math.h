 ...
 * @Last Modified time: 2024-01-07 09:19:57
 */
...
namespace xviz
{
    ...
    struct Mat22
    {
        ...
        Vec2f Solve(const Vec2f &b) const
        {
    ...
    struct Mat33
    {
        ...
        Vec2f Solve22(const Vec2f &b) const
        {
            float a11 = ex.x, a12 = ey.x, a21 = ex.y, a22 = ey.y;
            float det = a11 * a22 - a12 * a21;
            if (det != 0.0f)
    ...
    {
        return Vec2f(Dot(v, A.ex), Dot(v, A.ey));
    }
    ...
    {
        return Vec2f(q.m_cos * v.x - q.m_sin * v.y, q.m_sin * v.x + q.m_cos * v.y);
    }
